...
dragGraph.prototype = {
  ...
  paint() {
    ...
    if (this.selected) {
      ...
      if (this.type === 'text') {
        ...
        this.ctx.drawImage('./icon/close.png', this.x - 10, this.y - 10, 22, 22);
        this.ctx.drawImage('./icon/scale.png', this.x + this.w - 15, this.y + this.h - 15, 22, 22);
      }
    }
  ...
  transform(px, py, x, y, currentGraph) {
    ...
    if (this.type === 'image') {
      var rate=(currentGraph.w/currentGraph.h);
      var ww = lineB - lineA;
      const w = currentGraph.w +ww;
      const h = w/rate;
      this.w = w <= 0 ? 0 : w;
      this.h = h <= 0 ? 0 : h;
...
Component({
  ...
  methods: {
    ...
    draw() {
      this.ctx.clearRect(0, 0, this.data.screenWidth, this.data.screenHeight);
      this.ctx.setFillStyle("white");
      this.ctx.fillRect(0, 0, this.data.width, this.data.height);
      if (this.data.bgImage !== '') {
        let bgWidth = this.data.bgWidth;
        let bgHeight = this.data.bgHeight;
        if(bgWidth && bgHeight){//剪切画图
          if(bgWidth > bgHeight){
            let startX = (bgWidth-bgHeight)/2;
            this.ctx.drawImage(this.data.bgImage, startX, 0, bgHeight, bgHeight, 0, 0, this.data.width, this.data.height);
          }else{
            let startY = (bgHeight-bgWidth)/2;
            this.ctx.drawImage(this.data.bgImage, 0, startY, bgWidth, bgWidth, 0, 0, this.data.width, this.data.height);
          }
        }else{
    ...
    start(e) {
      ...
      this.drawArr && this.drawArr.forEach((item, index) => {
        var selflag=item.selected;
        item.selected =false;
        
        const action = item.isInGraph(x, y);
        if (action) {
          if (action === 'del' && selflag) {
    ...
    changeBgImage(url) {
      this.data.bgWidth = 0;
      this.data.bgHeight = 0;
      this.data.bgColor = '';
      this.data.bgImage = url;
      this.draw();
    },

    changeBgImageCut(url,bgWidth,bgHeight) {
      this.data.bgColor = '';
      this.data.bgImage = url;
